...
export function extractEntities(text = '') {
  ...
  pushMatches(/\b(?:PT|CV|UD|RS|Universitas|Institut|Politeknik)\s+[A-Z][A-Za-z0-9.&' -]{2,50}?(?=\s+(?:di|pada|untuk|dengan|yang|akan)\b|[,.;:\n]|$)/g, 'ORGANISASI');
...
export function generateEvidence(analysis, options = {}) {
  if (!analysis || !analysis.rows?.length) return [];
  const total = analysis.rows.length;
  const unit = options.isDocument ? 'segmen teks' : 'teks';
  ...
  const out = [];
  if (!options.isDocument && topCat && topCat.name !== 'Lainnya') out.push(`Kategori paling dominan adalah “${topCat.name}” dengan ${topCat.count} dari ${total} ${unit} (${topCat.pct}%).`);
  out.push(`Sentimen negatif ditemukan pada ${neg} dari ${total} ${unit} (${Math.round(neg / total * 100)}%), sedangkan sentimen positif pada ${pos} ${unit} (${Math.round(pos / total * 100)}%).`);
...
export function generateInsightSummary(analysis, dataPeriod = '', options = {}) {
  if (!analysis?.rows?.length) return 'Belum ada data yang dapat diringkas.';
  const total = analysis.rows.length;
  const unit = options.isDocument ? 'segmen teks dari dokumen' : 'teks';
  const topCat = analysis.categoriesRanked?.[0];
  ...
  const findings = [];
  if (!options.isDocument && topCat && topCat.name !== 'Lainnya') findings.push(`topik/kategori yang paling dominan adalah ${topCat.name} (${topCat.pct}%)`);
  findings.push(`kecenderungan sentimen ${dominantSentiment[0]} muncul pada ${Math.round(dominantSentiment[1] / total * 100)}% data`);
  if (topKeywords.length) findings.push(`kata kunci yang menonjol meliputi ${topKeywords.join(', ')}`);

  return `Analisis terhadap ${total} ${unit}${dataPeriod ? ` pada kurun waktu ${dataPeriod}` : ''} menunjukkan ${findings.join('; ')}.`;
}

export function generateRecommendations(analysis, options = {}) {
  if (!analysis?.rows?.length) return [];
  const top = analysis.categoriesRanked?.[0]?.name || '';
  const isDocument = options.isDocument === true;
  const actionMap = {
  ...
  const out = [];
  if (isDocument) {
    const topTerms = (analysis.keywords || []).slice(0, 3).map(x => x.term).join(', ');
    if (topTerms) out.push(`Tinjau bagian dokumen yang berkaitan dengan istilah dominan: ${topTerms}. Gunakan ringkasan sebagai pintu masuk, bukan pengganti pembacaan sumber asli.`);
    if (analysis.entitiesRanked?.length) out.push('Periksa kembali entitas penting seperti organisasi, lokasi, tanggal, atau nilai yang terdeteksi sebelum digunakan dalam laporan atau keputusan.');
    out.push('Cocokkan wawasan otomatis dengan bagian asli dokumen untuk memastikan konteks, pengecualian, dan ketentuan penting tidak terlewat.');
    return out;
  }
