...
function ensurePdfJsCompatibility() {
  // PDF.js 6 memakai API JavaScript modern. Beberapa browser/WebView yang
  // lebih lama belum menyediakan API berikut, sehingga perlu fallback.
  ...
  // Fallback tambahan untuk engine yang belum mendukung Map upsert proposal.
  // Build legacy PDF.js seharusnya sudah menangani kompatibilitas ini, tetapi
  // fallback ini menjaga proses import tetap aman pada WebView yang tertinggal.
...
export async function extractPdfTexts(file) {
  ...
  // Penting: pasang kompatibilitas SEBELUM pdfjs-dist dievaluasi.
  ensurePdfJsCompatibility();

  // Gunakan build legacy resmi PDF.js. Build ini ditujukan untuk browser atau
  // WebView yang belum memiliki seluruh API JavaScript modern yang dipakai
  // build utama PDF.js. Library dan worker harus berasal dari build yang sama.
